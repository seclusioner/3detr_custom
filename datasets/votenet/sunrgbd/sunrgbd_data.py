...
def data_viz2d(data_dir, dump_dir=os.path.join(BASE_DIR, 'data_viz2d')):
    ...
    for idx in range(len(sunrgbd)):
        ...
        # Loop over the objects and draw 2D bounding boxes
        for i, obj in enumerate(objects):
            # Compute 3D box corners and project them to 2D
            corners_3d_image, corners_3d = sunrgbd_utils.compute_box_3d(obj, calib)
            corners_2d = []
            for corner in corners_3d:
                # Project the 3D corner to 2D using the camera calibration
                uv, _ = calib.project_upright_depth_to_image(np.expand_dims(corner, axis=0).astype(np.float32))
                corners_2d.append([int(np.round(uv[0, 0])), int(np.round(uv[0, 1]))])
            
            # Draw the 2D bounding box by connecting the projected 3D corners
            corners_2d = np.array(corners_2d)
            # Draw lines connecting the 2D corners to form the bounding box
            for i in range(4):
                pt1 = tuple(corners_2d[i])
                pt2 = tuple(corners_2d[(i + 1) % 4])
                cv2.line(img, pt1, pt2, (0, 255, 0), 2)  # Top face of the box
            
            # Draw lines for the bottom face of the bounding box
            for i in range(4, 8):
                pt1 = tuple(corners_2d[i])
                pt2 = tuple(corners_2d[(i + 1) % 4 + 4])
                cv2.line(img, pt1, pt2, (0, 255, 0), 2)
            
            # Draw vertical lines connecting the top and bottom faces
            for i in range(4):
                pt1 = tuple(corners_2d[i])
                pt2 = tuple(corners_2d[i + 4])
                cv2.line(img, pt1, pt2, (0, 255, 0), 2)  # Vertices between top and bottom faces

            label_position = (corners_2d[0][0], corners_2d[0][1] - 10)  # Place label above the top-left corner
            cv2.putText(img, obj.classname, label_position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        Image.fromarray(img).save(os.path.join(dump_dir, 'img_box3d_2d.jpg'))
        print('-'*30)
        print('Type anything to continue to the next sample...')
        input()
...
def extract_sunrgbd_data(idx_filename, split, output_folder, num_point=20000,
    ...
    for data_idx in data_idx_list:
        ...
        for obj in objects:
            ...
            obb[7] = sunrgbd_utils.type2class[obj.classname] # name --> number
        ...
        pc_upright_depth = dataset.get_depth(data_idx) # (324314, 6)
        pc_upright_depth_subsampled = pc_util.random_sampling(pc_upright_depth, num_point) # (50000, 6)
        ...
        if save_votes:
            ...
            for obj in objects:
                ...
                try:
                    ...
                    # True: 1, False: 0 (for first dim)
                    point_votes[inds,0] = 1

                    # Add the votes (all 0 if the point is not in any object's OBB)
                    # vote = [cx - px, cy - py, cz - pz] (點到質心的向量)
                    votes = np.expand_dims(obj.centroid,0) - pc_in_box3d[:,0:3] # (4416, 3)
...
if __name__=='__main__':
    ...
    parser.add_argument('--viz2d', action='store_true', help='Run data visualization.')
    ...
    if args.viz2d:
        data_viz2d(os.path.join(BASE_DIR, 'sunrgbd_trainval'))
        exit()
