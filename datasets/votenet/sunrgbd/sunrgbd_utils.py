...
# Label format
"""
SUN RGBD: classname, box2d(1), box2d(2), box2d(3), box2d(4), centroid(1), centroid(2), centroid(3), 
coeffs(1), coeffs(2), coeffs(3), orientation(1), orientation(2)
"""
class SUNObject3d(object):
    ...
    ########## Added: to display lable object ##########
    def __str__(self):
        return f"Classname: {self.classname}, Box2D: {self.box2d}, Centroid: {self.centroid}, " \
               f"Dimensions (w, l, h): ({self.w}, {self.l}, {self.h}), " \
               f"Orientation: {self.orientation}, Heading Angle: {self.heading_angle:.2f}"
...
# 根據物體的中心位置、大小和旋轉角度，計算出物體在 3D 空間中的八個頂點（即 3D 邊界框的角點）
# 這些頂點可以幫助確定物體的 3D 邊界範圍，並將其與點雲進行比對
# Return: 3x8 的矩陣，每一列(每個sublist)代表一個角點的 (x, y, z) 坐標
...
def draw_projected_box3d(image, qs, labels, color=(255,255,255), thickness=2):
    ...
    if labels:
        # Loop through each label and add it next to the corresponding bounding box
        for idx, label in enumerate(labels):
            label_position = (qs[7][0], qs[7][1] - 10)  # Place label above the top-left corner
            cv2.putText(image, label, label_position, cv2.FONT_HERSHEY_SIMPLEX, 1, color, thickness, cv2.LINE_AA)
