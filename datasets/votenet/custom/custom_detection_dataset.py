...
import pc_util
import custom_utils
from model_util_custom import PointCloudConfig

DC = PointCloudConfig() # dataset specific config
MAX_NUM_OBJ = 64 # maximum number of objects allowed per scene
MEAN_COLOR_RGB = np.array([0.5,0.5,0.5]) # sunrgbd color is in 0~1

class CustomDetectionVotesDataset(Dataset):
    def __init__(self, split_set='train', num_points=20000,
        use_color=False, use_height=False, augment=False, scan_idx_list=None):

        assert(num_points<=50000)
        self.data_path = os.path.join(ROOT_DIR,
            'custom/custom_pc_bbox_votes_50k_%s'%(split_set))

        self.raw_data_path = os.path.join(ROOT_DIR, 'custom/custom_trainval')
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            rot_mat = custom_utils.rotz(rot_angle)
        ...
        for i in range(bboxes.shape[0]):
            bbox = bboxes[i]
            corners_3d = custom_utils.my_compute_box_3d(bbox[0:3], bbox[3:6], bbox[6])
...
if __name__=='__main__':
    d = CustomDetectionVotesDataset(use_height=True, use_color=True, use_v1=True, augment=True)
    sample = d[200]
